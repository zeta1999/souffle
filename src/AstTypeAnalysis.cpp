...
// TODO: make sure "getStoredType" not needed here (dont think it is) - why do we need it? check it's used correctly - put note on when to use it
void TypeSolution::generateConstraints() {
    ...
    public:
        // TODO: fix up this constructor once done (some args can be removed - just added for ease in the first run through)
        ConstraintFinder(std::set<std::unique_ptr<TypeConstraint>>& constraints, TypeLattice* lattice, TypeSolution* sol, AstProgram* program) : constraints(constraints), lattice(lattice), sol(sol), program(program) {}
        ...
        void visitCounter(const AstCounter& counter) {
            ...
            constraints.insert(std::make_unique<FixedConstraint>(&counter, ConstantAnalysisType(Kind::NUMBER)));
        }

        void visitNumberConstant(const AstNumberConstant& constant) {
            // number constants must actually be numbers
            constraints.insert(std::make_unique<FixedConstraint>(&constant, ConstantAnalysisType(Kind::NUMBER)));
        }

        void visitStringConstant(const AstStringConstant& constant) {
            // string constants must actually be strings
            constraints.insert(std::make_unique<FixedConstraint>(&constant, ConstantAnalysisType(Kind::SYMBOL)));
        }

        void visitNullConstant(const AstNullConstant& constant) {
            // nils must be record types
            constraints.insert(std::make_unique<FixedConstraint>(&constant, ConstantAnalysisType(Kind::RECORD)));
        }
        ...
        void visitTypeCast(const AstTypeCast& cast) {
            ...
            constraints.insert(std::make_unique<FixedConstraint>(&cast, std::unique_ptr<AnalysisType>(type->clone())));
        }
        ...
        void visitIntrinsicFunctor(const AstIntrinsicFunctor& functor) {
            ...
            if (functor.getFunction() == FunctorOp::MAX || functor.getFunction() == FunctorOp::MIN) {
                ...
                auto constantConstraint = std::make_unique<ImplicationConstraint>(std::make_unique<FixedConstraint>(&functor, std::make_unique<ConstantAnalysisType>(kind)));
                for (size_t i = 0; i < functor.getArity(); i++) {
                    ...
                    if (functor.acceptsSymbols(i)) {
                        constantConstraint->addRequirement(std::make_unique<FixedConstraint>(arg, std::make_unique<ConstantAnalysisType>(Kind::SYMBOL)));
                    } else if (functor.acceptsNumbers(i)) {
                        constantConstraint->addRequirement(std::make_unique<FixedConstraint>(arg, std::make_unique<ConstantAnalysisType>(Kind::NUMBER)));
                    } else {
        ...
        void visitAtom(const AstAtom& atom) {
            ...
            for (size_t i = 0; i < atom.getArity(); i++) {
                ...
                const AnalysisType* expectedType = lattice->getAnalysisType(rel->getAttribute(i)->getTypeName());
                constraints.insert(std::make_unique<FixedConstraint>(arg, std::unique_ptr<AnalysisType>(expectedType->clone())));
            }
        }

...
protected:
    ...
    struct node : public base {
        ...
        static constexpr size_t split_point = std::min(3 * maxKeys / 4, maxKeys - 2);
        static constexpr size_t minKeys = std::min(maxKeys - (split_point + 1), split_point + 1);
    ...
    public:
        ...
        std::vector<chunk>& collectChunks(
                std::vector<chunk>& res, size_type num, const iterator& begin, const iterator& end) {
            assert(num > 0);
...
public:
    ...
    iterator end() const {
        ...
        if (rightmost) {
            return iterator(rightmost, rightmost->getNumElements());
        } else {
            return iterator();
        }
    }

...
void SynthesiserDirectRelation::generateTypeStruct(std::ostream& out) {
    ...
    for (size_t i = 0; i < numIndexes; i++) {
        out << "range<t_ind_" << i << "::iterator> lowerUpperBound("
            << "const t_tuple& low, const t_tuple& high, t_ind_" << i << "::operation_hints& h) const {\n";
...
/** Generate index set for a hashset relation */
void SynthesiserHashsetRelation::computeIndices() {
    assert(!isProvenance && "hashset cannot be used with provenance");
...
/** Generate type name of a hashset relation */
std::string SynthesiserHashsetRelation::getTypeName() {
    std::stringstream res;
    res << "t_hashset_" << getArity();
...
/** Generate type struct of a hashset relation */
void SynthesiserHashsetRelation::generateTypeStruct(std::ostream& out) {
    size_t arity = getArity();
    const auto& inds = getIndices();
    size_t numIndexes = inds.size();
    std::map<std::vector<int>, int> indexToNumMap;
    ...
    for (size_t i = 0; i < inds.size(); i++) {
        ...
        // generate a custom tuple hasher based on the index order
        out << "struct tuple_hasher_" << i << " {\n";
        out << "bool operator()(const t_tuple& a) const {\n";
        out << "size_t h = a[" << ind[ind.size() - 1] << "];\n";
        for (int i = ind.size() - 2; i >= 0; i--) {
            out << "h = a[" << ind[i] << "] + 0x9e3779b9 + (h << 6) + (h >> 2);\n";
        }
        out << "return h;\n";
        out << "}\n";
        out << "};\n";

        // generate a custom index comparator
        out << "struct tuple_equal_" << i << " {\n";
        out << "bool operator()(const t_tuple& a, const t_tuple& b) const {\n";
        out << "return ";
        for (size_t i = 0; i < ind.size() - 1; i++) {
            out << "a[" << ind[i] << "] == b[" << ind[i] << "] &&\n";
    ...
    // printHintStatistics method
    out << "void printHintStatistics(std::ostream& o, const std::string prefix) const {\n";
    out << "o << \"hashset index: no hint statistics supported\\n\";\n";
    out << "}\n";

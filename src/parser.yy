 ...
 *
 ***********************************************************************/
% skeleton "lalr1.cc" % require "3.0.2"

        % defines % define parser_class_name{parser} % define api.token.constructor %
        define api.value.type variant % define parse.assert % define api.location.type{SrcLocation}
...
#include "ParserDriver.h"
        ...
        /* -- Tokens -- */
        % token END 0 "end of file" % token<std::string> STRING "symbol" %
        token<std::string> IDENT "identifier" % token<RamDomain> NUMBER "number" %
        token<RamFloat> FLOAT "float" % token<std::string> RELOP "relational operator" %
        token PRAGMA "pragma directive" % token OUTPUT_QUALIFIER "relation qualifier output" %
        ...
        token INLINE_QUALIFIER "relation qualifier inline" % token TMATCH "match predicate" %
        token TCONTAINS "checks whether substring is contained in a string" %
        token CAT "concatenation of strings" % token ORD "ordinal number of a string" %
        token STRLEN "length of a string" % token SUBSTR "sub-string of a string" %
        token MIN "min aggregator" % token MAX "max aggregator" % token COUNT "count aggregator" %
        token SUM "sum aggregator" % token TRUE "true literal constraint" %
        token FALSE "false literal constraint" % token PLAN "plan keyword" % token IF ":-" %
        token DECL "relation declaration" % token FUNCTOR "functor declaration" %
        token INPUT_DECL "input directives declaration" % token OUTPUT_DECL "output directives declaration" %
        token PRINTSIZE_DECL "printsize directives declaration" %
        token OVERRIDE "override rules of super-component" % token TYPE "type declaration" %
        token COMPONENT "component declaration" % token INSTANTIATE "component instantiation" %
        token NUMBER_TYPE "numeric type declaration" % token SYMBOL_TYPE "symbolic type declaration" %
        token TONUMBER "convert string to (signed) number" % token TOSTRING "convert number to string" %
        token ITOU "convert int to unsigned" % token ITOF "convert int to float" %
        token UTOI "convert unsigned to int" % token UTOF "convert unsigned to float" %
        token FTOI "convert float to int" % token FTOU "convert float to unsigned" % token AS "type cast" %
        token AT "@" % token NIL "nil reference" % token PIPE "|" % token LBRACKET "[" % token RBRACKET "]" %
        token UNDERSCORE "_" % token DOLLAR "$" % token PLUS "+" % token MINUS "-" % token EXCLAMATION "!" %
        token LPAREN "(" % token RPAREN ")" % token COMMA "," % token COLON ":" % token DOUBLECOLON "::" %
        token SEMICOLON ";" % token DOT "." % token EQUALS "=" % token STAR "*" % token SLASH "/" %
        token CARET "^" % token PERCENT "%" % token LBRACE "{" % token RBRACE "}" % token LT "<" %
        token GT ">" % token BW_AND "band" % token BW_OR "bor" % token BW_XOR "bxor" % token BW_NOT "bnot" %
        token L_AND "land" % token L_OR "lor" %
        token L_NOT "lnot"

        /* -- Non-Terminal Types -- */
        % type<AstAtom*> atom % type<AstArgument*> arg % type<RuleBody*> body %
        type<AstComponentType*> comp_type % type<AstComponentInit*> comp_init %
        type<AstComponent*> component % type<AstComponent*> component_body %
        type<AstComponent*> component_head % type<RuleBody*> conjunction % type<AstConstraint*> constraint %
        type<RuleBody*> disjunction % type<AstExecutionOrder*> exec_order_list %
        type<AstExecutionPlan*> exec_plan % type<AstExecutionPlan*> exec_plan_list % type<AstClause*> fact %
        type<AstFunctorDeclaration*> functor_decl % type<TypeAttribute> functor_type %
        type<std::vector<AstAtom*>> head % type<std::vector<std::string>> identifier %
        type<std::vector<AstIO*>> io_directive_list % type<std::vector<AstIO*>> io_relation_list %
        type<std::string> kvp_value % type<std::vector<AstIO*>> io_head %
        ...
        type<AstRecordType*> non_empty_record_type_list % type<AstPragma*> pragma %
        type<std::set<RelationTag>> relation_tags % type<std::vector<AstRelation*>> relation_decl %
        type<std::vector<AstRelation*>> relation_list % type<std::vector<AstClause*>> rule %
        type<std::vector<AstClause*>> rule_def % type<RuleBody*> term % type<AstType*> type %
...
comp_type % destructor {
    delete $$;
}
comp_init % destructor {
    delete $$;
}
component_body % destructor {
    delete $$;
}
component_head % destructor {
    delete $$;
}
...
disjunction % destructor {
    delete $$;
}
exec_order_list % destructor {
    delete $$;
}
exec_plan % destructor {
    delete $$;
}
exec_plan_list % destructor {
    delete $$;
}
fact % destructor {
    delete $$;
}
functor_decl % destructor {}
functor_type % destructor {
    for (auto* cur : $$) {
        delete cur;
    }
}
...
io_directive_list % destructor {
    for (auto* cur : $$) {
        delete cur;
    }
}
io_relation_list % destructor {
    for (auto* cur : $$) {
        delete cur;
    }
}
non_empty_arg_list % destructor {
    for (auto* cur : $$) {
        delete cur;
    }
}
non_empty_attributes % destructor {
    delete $$;
}
non_empty_exec_order_list % destructor {}
non_empty_functor_arg_type_list % destructor {}
non_empty_key_value_pairs % destructor {
    delete $$;
}
non_empty_record_type_list % destructor {
    delete $$;
}
pragma % destructor {}
relation_tags % destructor {
    for (auto* cur : $$) {
        delete cur;
    }
}
relation_decl % destructor {
    for (auto* cur : $$) {
        delete cur;
    }
}
relation_list % destructor {
    for (auto* cur : $$) {
        delete cur;
    }
}
...
type % destructor {}
type_params % destructor {}
type_param_list % destructor {
    delete $$;
}
union_type_list

        /* -- Operator precedence -- */
        % left L_OR % left L_AND % left BW_OR % left BW_XOR % left BW_AND % left PLUS MINUS %
        left STAR SLASH PERCENT % precedence NEG BW_NOT L_NOT %
        right CARET
...
non_empty_record_type_list : IDENT COLON identifier {
    $$ = new AstRecordType();
...
relation_decl : DECL relation_list LPAREN RPAREN relation_tags {
...
non_empty_attributes : IDENT COLON identifier {
...
/* Relation tags */
relation_tags : relation_tags OUTPUT_QUALIFIER {
    driver.warning(@2, "Deprecated output qualifier used");
    if ($1.find(RelationTag::OUTPUT) != $1.end()) driver.error(@2, "output qualifier already set");
    $1.insert(RelationTag::OUTPUT);
    $$ = $1;
}
| relation_tags INPUT_QUALIFIER {
    driver.warning(@2, "Deprecated input qualifier was used");
    if ($1.find(RelationTag::INPUT) != $1.end()) driver.error(@2, "input qualifier already set");
    $1.insert(RelationTag::INPUT);
    $$ = $1;
}
| relation_tags PRINTSIZE_QUALIFIER {
    driver.warning(@2, "Deprecated printsize qualifier was used");
    if ($1.find(RelationTag::PRINTSIZE) != $1.end()) driver.error(@2, "printsize qualifier already set");
    $1.insert(RelationTag::PRINTSIZE);
    $$ = $1;
}
| relation_tags OVERRIDABLE_QUALIFIER {
    if ($1.find(RelationTag::OVERRIDABLE) != $1.end()) driver.error(@2, "overridable qualifier already set");
    $1.insert(RelationTag::OVERRIDABLE);
    $$ = $1;
}
| relation_tags INLINE_QUALIFIER {
    if ($1.find(RelationTag::INLINE) != $1.end()) driver.error(@2, "inline qualifier already set");
    $1.insert(RelationTag::INLINE);
    $$ = $1;
}
| relation_tags BRIE_QUALIFIER {
    if ($1.find(RelationTag::BRIE) != $1.end() || $1.find(RelationTag::BTREE) != $1.end() ||
...
exec_plan_list : NUMBER COLON LPAREN exec_order_list RPAREN {
...
exec_order_list : non_empty_exec_order_list {
...
non_empty_exec_order_list : NUMBER {
    $$ = new AstExecutionOrder();
...
/* Rule body atom */
atom : identifier LPAREN non_empty_arg_list RPAREN {
    $$ = new AstAtom();
...
constraint
    ...
    $$ = new AstBinaryConstraint(
            $RELOP, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] LT arg[right] {
    $$ = new AstBinaryConstraint(BinaryConstraintOp::LT, std::unique_ptr<AstArgument>($left),
...
| arg[left] GT arg[right] {
    $$ = new AstBinaryConstraint(BinaryConstraintOp::GT, std::unique_ptr<AstArgument>($left),
            std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] EQUALS arg[right] {
    $$ = new AstBinaryConstraint(BinaryConstraintOp::EQ, std::unique_ptr<AstArgument>($left),
            std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| TMATCH LPAREN arg[left] COMMA arg[right] RPAREN {
    $$ = new AstBinaryConstraint(BinaryConstraintOp::MATCH, std::unique_ptr<AstArgument>($left),
            std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| TCONTAINS LPAREN arg[left] COMMA arg[right] RPAREN {
    $$ = new AstBinaryConstraint(BinaryConstraintOp::CONTAINS, std::unique_ptr<AstArgument>($left),
            std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| MINUS arg[nested_arg] % prec NEG {
    if (const AstNumberConstant* original = dynamic_cast<const AstNumberConstant*>($nested_arg)) {
        ...
        $$ = new AstIntrinsicFunctor(FunctorOp::NEG, std::unique_ptr<AstArgument>($nested_arg));
        $nested_arg = nullptr;
        $$->setSrcLoc(@$);
    }
}
| BW_NOT arg[nested_arg] {
    $$ = new AstIntrinsicFunctor(FunctorOp::BNOT, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| L_NOT arg[nested_arg] {
    $$ = new AstIntrinsicFunctor(FunctorOp::LNOT, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| ORD LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::ORD, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| STRLEN LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::STRLEN, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| TONUMBER LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::TONUMBER, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| TOSTRING LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::TOSTRING, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| ITOU LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::ITOU, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| ITOF LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::ITOF, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| UTOI LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::UTOI, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| UTOF LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::UTOF, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| FTOI LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::FTOI, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);

    $nested_arg = nullptr;
}
| FTOU LPAREN arg[nested_arg] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::FTOU, std::unique_ptr<AstArgument>($nested_arg));
    $$->setSrcLoc(@$);
...
| arg[left] PLUS arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::ADD, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] MINUS arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::SUB, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] STAR arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::MUL, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] SLASH arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::DIV, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] PERCENT arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::MOD, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] CARET arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::EXP, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] BW_OR arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::BOR, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] BW_XOR arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::BXOR, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] BW_AND arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::BAND, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] L_OR arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::LOR, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| arg[left] L_AND arg[right] {
    $$ = new AstIntrinsicFunctor(
            FunctorOp::LAND, std::unique_ptr<AstArgument>($left), std::unique_ptr<AstArgument>($right));
    $$->setSrcLoc(@$);
...
| SUBSTR LPAREN arg[first] COMMA arg[second] COMMA arg[third] RPAREN {
    $$ = new AstIntrinsicFunctor(FunctorOp::SUBSTR, std::unique_ptr<AstArgument>($first),
            std::unique_ptr<AstArgument>($second), std::unique_ptr<AstArgument>($third));
    $$->setSrcLoc(@$);
...
/* Component body */
component_body : component_body[comp] type {
    $$ = $comp;
...
/* Functor declaration */
functor_decl : FUNCTOR IDENT LPAREN RPAREN COLON functor_type {
...
non_empty_functor_arg_type_list : functor_type {
...
/* Pragma directives */
pragma : PRAGMA STRING[key] STRING[value] {
...
/* Store directives */
io_head : OUTPUT_DECL io_directive_list {
...
/* Key-value pairs */
non_empty_key_value_pairs : IDENT EQUALS kvp_value {

...
public:
     ...
     */
    RAMIRelation(std::size_t arity, const std::string& name, const std::vector<std::string>& attributeTypes,
            const MinIndexSelection& orderSet, IndexFactory factory = &createBTreeIndex);

    RAMIRelation(RAMIRelation& other) = delete;

    virtual ~RAMIRelation() = default;

    /**
     * Support for-each iteration for RAMIRelation.
     */
    ...
    public:
        ...
        Iterator(const RAMIRelation& rel) : stream(std::make_unique<Stream>(rel.scan())) {}
     ...
     */
    void insert(const RAMIRelation& other);
     ...
     */
    virtual void extend(const RAMIRelation& rel);
...
protected:
    ...
    std::vector<std::unique_ptr<RAMIIndex>> indexes;
...
public:
    RAMIEqRelation(size_t arity, const std::string& relName, const std::vector<std::string>& attributeTypes,
    ...
    void extend(const RAMIRelation& rel) override;
};

/**
 * Interpreter Indirect Relation
 */
class RAMIIndirectRelation : public RAMIRelation {
public:
    RAMIIndirectRelation(size_t arity, const std::string& relName,
